...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
            ...
            )
        {
            ...
            // x => x.Countries.Intersect(countries.ToList()).Any(), true
            var countryFilter = Builders<Movie>.Filter.In( "Countries", countries);

            var sortTitleDescend = Builders<Movie>.Sort.Descending(x => x.Title);

            var projectionFilter = Builders<Movie>.Projection.Include(x => x.Title);           

            return await _moviesCollection
               .Find(countryFilter)
               .Sort(sortTitleDescend)
            ...
            int page = 0, params string[] genres)
        {
            ...
            var filterByGenre = Builders<Movie>.Filter.In("Genres", genres);

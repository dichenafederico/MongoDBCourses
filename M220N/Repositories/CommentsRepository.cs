...
namespace M220N.Repositories
{
    public class CommentsRepository
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return await _commentsCollection.UpdateOneAsync(
            Builders<Comment>.Filter.Where(c => c.Id == commentId && c.MovieId == movieId && c.Email == user.Email),
        ...
        {         
            ...
            */
            try
            {
                ...
                var projectionFilter =  Builders<BsonDocument>.Projection.Include("count").Include("_id");

                result =  _commentsCollection
                       .WithReadConcern(ReadConcern.Majority)
                       .AsQueryable()
                       .GroupBy(c => c.Email).Select(g => new ReportProjection
                       {
                           Count = g.Count(),
                           Id = g.First().Email
                       ...
                       //.Limit(20).Project<ReportProjection>(projectionFilter).ToListAsync();

...
namespace M220N.Repositories
{
    public class UsersRepository
    {
        ...
        */
        ...
        {
            var filterByEmail = Builders<User>.Filter.Eq("Email", email);

            return await _usersCollection
               .Find(filterByEmail).SingleAsync(cancellationToken);
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var filerEmail = Builders<Session>.Filter.Eq("Email", storedUser.Email);

                var sessionUpdateUser = Builders<Session>.Update.Set(s => s.UserId , storedUser.Id).Set( s => s.Jwt , user.AuthToken);

                var options = new FindOneAndUpdateOptions<Session> { IsUpsert = true };

                await _sessionsCollection.FindOneAndUpdateAsync(filerEmail, sessionUpdateUser, options);
        ...
        public async Task<UserResponse> LogoutUserAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            var filterEmail = Builders<Session>.Filter.Eq("Email", email);

            await _sessionsCollection.FindOneAndDeleteAsync(filterEmail);
            ...
            Dictionary<string, string> preferences, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                */
                ...
                var updateUserPreferences = Builders<User>.Update.Set(u => u.Preferences, preferences);

                updateResult = await _usersCollection.UpdateOneAsync(
                   new BsonDocument(),
                   Builders<User>.Update.Set("Prefrences", preferences),
